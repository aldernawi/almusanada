...
@section('page_title', 'Reviewers & Viewers')

@section('content')
    <div class="container mx-auto px-4 py-8 max-w-5xl">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Manage Reviewers & Viewers</h1>
                <p class="text-gray-600 mt-2">Create and manage reviewer and viewer (view-only) accounts</p>
            </div>
        ...
        @if(session('success'))
            <div
                class="bg-green-50 border-l-4 border-green-500 text-green-800 px-6 py-4 rounded-lg mb-6 flex items-center gap-3">
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="bg-gray-50 border-b">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Permissions</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Created Date</th>
                ...
                <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                @if($reviewer->role === 'viewer')
                                    <span class="bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-xs">Viewer</span>
                                @else
                                    <span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">Reviewer</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                @if($reviewer->role === 'viewer')
                                    <span class="bg-gray-100 text-gray-500 px-2 py-1 rounded text-xs">View Only</span>
                                @elseif($reviewer->can_view_all_transactions)
                                    <span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">All Transactions</span>
                                @else
                                    <span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs">Limited</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">No accounts found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
