...
@section('page_title', 'المعاملات')

@section('content')
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-800">إدارة المعاملات</h1>
                <p class="text-slate-500 mt-1 font-medium">عرض وإدارة الحالات وأرقام التتبع</p>
            </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase">صاحب المعاملة</th>
                        <th class="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase">العميل (الحساب)</th>
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="table-row hover:bg-slate-50 transition">
                            <td class="px-6 py-4 font-bold text-blue-600 flex items-center gap-2">
                                ...
                                @if($transaction->pdf_path)
                                    <a href="{{ asset($transaction->pdf_path) }}" target="_blank" class="text-red-500 hover:text-red-700" title="تحميل PDF">
                            ...
                            <td class="px-6 py-4 text-slate-600">
                                ...
                                @else
                                    <span class="text-slate-400 text-xs">غير مربوط</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-xs font-bold
                                            @if($transaction->status == 'تمت') bg-emerald-100 text-emerald-700
                                            @elseif($transaction->status == 'جاهزة للاستلام') bg-blue-100 text-blue-700
                                            @elseif($transaction->status == 'مرفوضة') bg-red-100 text-red-700
                                            @elseif($transaction->status == 'موقوفة') bg-slate-200 text-slate-700
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center gap-3">
                                    <button onclick="showStatusModal({{ $transaction->id }}, '{{ $transaction->status }}')"
                                        class="text-emerald-600 hover:text-emerald-800 font-semibold text-sm transition flex items-center gap-1"><i class="fas fa-exchange-alt text-xs"></i> الحالة</button>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400">لا توجد معاملات حالياً</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="statusModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50">
        <div class="bg-white rounded-2xl p-6 w-96 shadow-2xl" style="animation: contentSlide 0.3s ease;">
            <h3 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><i class="fas fa-exchange-alt text-blue-600"></i> تغيير حالة المعاملة</h3>
            <form id="statusForm" action="{{ route('admin.transactions.update.status') }}" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">الحالة الجديدة</label>
