...
<div class="container mx-auto px-4 py-8 max-w-3xl">
    ...
    <div class="main-card p-8">
        <h1 class="text-2xl font-extrabold text-slate-800 mb-2 pb-4 border-b border-slate-100 flex items-center gap-3"><i class="fas fa-edit text-blue-600"></i> Edit Transaction</h1>
        <p class="text-blue-600 font-bold mb-6">Transaction #: {{ $transaction->transaction_number }}</p>
        ...
        <form action="{{ route('admin.transactions.update', $transaction->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Customer (Beneficiary)</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Update PDF File (Optional)</label>
                    <input type="file" name="pdf" accept="application/pdf"
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition outline-none">
                    @if($transaction->pdf_path)
                        <div class="mt-2 flex items-center gap-2 text-sm text-emerald-600">
                            <i class="fas fa-file-pdf"></i>
                            <a href="{{ asset($transaction->pdf_path) }}" target="_blank" class="underline font-semibold">View current file</a>
                        </div>
                    @endif
