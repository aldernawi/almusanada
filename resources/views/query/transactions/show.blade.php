...
<head>
    ...
    <title>تفاصيل المعاملة - {{ $transaction->transaction_number }}</title>
    ...
    <header class="bg-white border-b border-slate-100 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <img src="{{ asset('images/logo.png') }}" alt="المُساندة" class="h-10 w-auto object-contain">
                <div class="w-px h-8 bg-slate-200"></div>
                <h1 class="text-lg font-bold text-slate-800">تفاصيل المعاملة</h1>
            </div>
            <div class="flex items-center gap-4">
            ...
            @if($transaction->pdf_path)
                <div class="mt-6">
                    <a href="{{ asset($transaction->pdf_path) }}" target="_blank" class="inline-flex items-center gap-2 px-6 py-3 bg-red-50 text-red-600 rounded-xl font-semibold text-sm hover:bg-red-100 transition">
                        <i class="fas fa-file-pdf text-lg"></i> عرض الملف المرفق
                    </a>
                </div>
            @endif
        </div>
