...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .orb-3 { top: 40%; left: 50%; width: 400px; height: 400px; background: radial-gradient(circle, rgba(59, 130, 246, 0.15) 0%, transparent 70%); animation-delay: -14s; }
        ...
        .login-wrapper { width: 100%; max-width: 440px; position: relative; z-index: 10; animation: slideUp 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
        @keyframes slideUp { from { opacity: 0; transform: translateY(40px) scale(0.96); } to { opacity: 1; transform: translateY(0) scale(1); } }

        .login-brand { text-align: center; margin-bottom: 2rem; }
        .logo-wrap { display: inline-block; margin-bottom: 1rem; animation: gentlePulse 4s infinite ease-in-out; }
        ...
        .login-brand h1 { font-size: 1.6rem; font-weight: 800; color: white; letter-spacing: 0.5px; }
        .login-brand p { font-size: 0.9rem; color: rgba(255, 255, 255, 0.5); margin-top: 0.3rem; font-weight: 500; }

        .login-card {
            background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(248, 250, 252, 0.98) 100%); backdrop-filter: blur(20px);
            border-radius: 24px; padding: 2.5rem;
            box-shadow: 0 30px 80px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(255, 255, 255, 0.1), 0 0 60px rgba(37, 99, 235, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.6);
            position: relative; overflow: hidden; border: 1px solid rgba(37, 99, 235, 0.08);
        }
        .login-card::before {
        ...
        .card-title { font-size: 1.3rem; font-weight: 800; color: #0f172a; margin-bottom: 0.3rem; display: flex; align-items: center; gap: 0.6rem; }
        .card-title::before { content: ''; width: 4px; height: 24px; background: linear-gradient(180deg, #2563eb, #1d4ed8); border-radius: 2px; flex-shrink: 0; }
        .card-subtitle { font-size: 0.875rem; color: #94a3b8; margin-bottom: 2rem; font-weight: 500; }

        .error-box {
            background: linear-gradient(135deg, #fef2f2, #fee2e2); border: 1px solid #fecaca;
            border-radius: 12px; padding: 0.875rem 1.25rem; margin-bottom: 1.5rem; animation: shake 0.4s ease;
        }
        @keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-6px); } 75% { transform: translateX(6px); } }
        .error-box ul { list-style: none; }
        .error-box li { font-size: 0.85rem; color: #991b1b; font-weight: 600; display: flex; align-items: center; gap: 0.5rem; }
        ...
        .remember-row input[type="checkbox"] { width: 18px; height: 18px; accent-color: #2563eb; cursor: pointer; border-radius: 5px; }
        .remember-row label { font-size: 0.85rem; color: #64748b; cursor: pointer; font-weight: 600; }
        ...
        .btn-login::before { content: ''; position: absolute; top: 0; left: -100%; width: 60%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.25), transparent); transition: left 0.6s ease; z-index: 1; }
        ...
        @media (max-width: 480px) { .login-card { padding: 2rem 1.5rem; } .login-wrapper { max-width: 100%; } }
    </style>
</head>
    ...
    <div class="login-wrapper">
        ...
        <div class="login-card">
            ...
            @if(session('error'))
                <div class="error-box">
                    <ul>
                        <li><i class="fas fa-exclamation-circle"></i> {{ session('error') }}</li>
                    </ul>
                </div>
            @endif
